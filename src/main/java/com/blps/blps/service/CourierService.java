...
public class CourierService {
    ...
    public RestaurantOrCourierOrderActionResponse pickUpOrder(Long orderId, Long courierId) {
        Order order = orderRepository.findByIdAndCourierId(orderId, courierId)
                .orElseThrow(() -> new ResourceNotFoundException("Заказ не найден или не принадлежит курьеру"));

        if (order.getStatus() != OrderStatus.READY) {
            throw new BusinessException("Заказ можно забрать только в статусе 'ГОТОВ'. Текущий статус: " + order.getStatus());
        }

        order.setStatus(OrderStatus.PICKED_UP);
        orderRepository.save(order);

        return new RestaurantOrCourierOrderActionResponse(order.getId(), order.getStatus().name(), "Заказ принят курьером");
    }
    ...
    public RestaurantOrCourierOrderActionResponse deliverOrder(Long orderId, Long courierId) {
        ...
        if (order.getStatus() != OrderStatus.PICKED_UP) {
            throw new BusinessException("Заказ можно доставить только после того, как он был принят курьером. Текущий статус: " + order.getStatus());
        }
        ...
        return new RestaurantOrCourierOrderActionResponse(order.getId(), order.getStatus().name(), "Заказ доставлен");
    }
